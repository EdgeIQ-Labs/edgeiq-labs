/**
 * POST /api/webhooks/stripe-brandguard
 *
 * Stripe webhook for BrandGuard checkout completion.
 * On checkout.session.completed:
 *   1. Stores brandguard:{email}:{domain} in KV
 *   2. Sends welcome email via Resend
 *
 * KV binding : PULSE_KV
 * Env vars   : STRIPE_BRANDGUARD_WEBHOOK_SECRET, RESEND_API_KEY, FROM_EMAIL, SITE_URL
 *
 * Register at: https://dashboard.stripe.com/webhooks
 * Events:      checkout.session.completed
 * Endpoint:    https://edgeiqlabs.com/api/webhooks/stripe-brandguard
 */
...
async function sendWelcomeEmail(env, email, domain) {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: email,
      subject: `BrandGuard activated for ${domain} — EdgeIQ Labs`,
...
<div style="max-width:580px;margin:0 auto;padding:32px 20px;">
  <div style="background:#121923;border:1px solid #1e2e3e;border-radius:16px;padding:36px 32px;text-align:center;">
    ...
    <h1 style="color:#e8eef7;font-size:1.5rem;margin:0 0 10px;">BrandGuard is watching</h1>
    <p style="color:#9fb0c7;font-size:0.9rem;margin:0 0 8px;">We're now monitoring lookalike domains for <strong style="color:#f472b6;">${domain}</strong>.</p>
    <p style="color:#9fb0c7;font-size:0.9rem;margin:0 0 24px;">Your first scan runs within the next few minutes. You'll hear from us the moment a suspicious domain surfaces — and weekly regardless.</p>
    <a href="${siteUrl}/account/" style="display:inline-block;background:#f472b6;color:#071018;font-weight:700;padding:12px 28px;border-radius:9px;text-decoration:none;">View BrandGuard Dashboard →</a>
...
export async function onRequestPost({ request, env }) {
  ...
  const valid = await verifyStripeSignature(rawBody, sigHeader, env.STRIPE_BRANDGUARD_WEBHOOK_SECRET);
  ...
  if (!email || !domain) {
    console.warn('BrandGuard webhook: missing email/domain in session', session.id);
