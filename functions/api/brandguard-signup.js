...
async function sendWelcomeEmail(env, email, domain) {
  ...
  const bg = '#0b0f14', card = '#121923', text = '#e8eef7', muted = '#9fb0c7', border = '#1e2e3e', pink = '#f472b6';
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    <tr><td style="padding:0 0 24px;"><span style="font-size:18px;font-weight:800;color:${text};">EdgeIQ<span style="color:${pink}"> Labs</span></span></td></tr>
    <tr><td style="background:${card};border:1px solid ${border};border-radius:14px;padding:32px;">
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:${pink};margin-bottom:12px;">🛡️ BrandGuard Active</div>
      <h1 style="font-size:22px;font-weight:800;color:${text};margin:0 0 12px;">Your brand is now being watched</h1>
      <p style="font-size:14px;color:${muted};line-height:1.7;margin:0 0 24px;">BrandGuard is now monitoring lookalike and typosquatted domains targeting <strong style="color:${text};">${domain}</strong>. We scan daily and will alert you the moment a new impostor appears.</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
        ${[
          ['Typosquatting','Domains with swapped, added, or missing letters (e.g. examp1e.com)'],
          ['Homoglyph attacks','Look-alike characters from other alphabets (e.g. Cyrillic "а")'],
          ['Keyword-append domains','Your brand + common words (e.g. yourbrand-login.com)'],
          ['TLD swaps','Your domain on .net, .co, .io, .info, and 50+ other TLDs'],
      ...
      <p style="font-size:13px;color:${muted};line-height:1.6;margin:0 0 20px;">Your first full scan runs within 24 hours. After that we scan daily and email you only when something new appears — no noise.</p>
      <a href="https://edgeiqlabs.com/account/" style="display:inline-block;background:${pink};color:#071018;font-weight:700;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none;">View your account →</a>
    </td></tr>
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: 'EdgeIQ Labs <security@edgeiqlabs.com>', to: [email], subject: `BrandGuard is now watching for impostor domains targeting ${domain}`, html }),
  }).catch(() => {});
}
